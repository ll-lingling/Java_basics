...
public class UserServiceImpl implements IUserService {
    ...
    public ServerResponse<String> updateUser(User user) {
        //1.校验用户名是否存在
        User userMsg = userMapper.queryUserById(String.valueOf(user.getId()));
        if(StringUtils.isNotBlank(user.getUsername())){
            int count = userMapper.checkUserName(user.getUsername());
            if(count >= 1 && !userMsg.getUsername().equals(user.getUsername())){
                return ServerResponse.createByErrorMsg("用户名已经存在");
            }
        }
        ...
        if(StringUtils.isNotBlank(user.getEmail())){
            ...
            if(count >= 1 && !userMsg.getEmail().equals(user.getEmail())){
    ...
    public ServerResponse<User> findUserById(String id) {
        ...
        return ServerResponse.createByErrorMsg("更新的用户不存在");
    }
